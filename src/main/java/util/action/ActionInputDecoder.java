...
public class ActionInputDecoder implements Runnable {
    ...
    /**
     * Creates a new viergewinnt.util.action from the given data. The decoding-overhead-data should be included in the argument array.
     *
     * @param data The read viergewinnt.util.action data (including the overhead).
    ...
    private Action createAction(byte[] data) {
        ...
        if (data[0] >= allTypes.length)
            throw new IllegalArgumentException("The viergewinnt.util.action given in the data is unknown. viergewinnt.util.action = " + data[0]);
        ...
        switch (type) {
            ...
            case undo:
                // TODO undo viergewinnt.util.action
            case message:
                // TODO message viergewinnt.util.action
            case error:
                // TODO error viergewinnt.util.action
            default:
                throw new IllegalArgumentException("Illegal viergewinnt.util.action type. type = " + type);
        }
    }
    ...
    /**
     * Gets the current viergewinnt.util.action.
     *
     * @return The current viergewinnt.util.action or {@code null} if there is none.
     */
            ...
            try {
                ...
                if (newDataStart) {
                    ...
                    if (currentDataPosition == 1) {
                        ...
                        if (readByte == 0) {
                            readData[currentDataPosition] = readByte;
                            // add the viergewinnt.util.action to the list (thread-safe!)
                            synchronized (this.actions) {
